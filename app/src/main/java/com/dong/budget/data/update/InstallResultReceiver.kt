...
class InstallResultReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (val status = intent.getIntExtra(PackageInstaller.EXTRA_STATUS, Int.MIN_VALUE)) {
            ...
            PackageInstaller.STATUS_SUCCESS -> {
                ...
                InstallEvents.publish(failureOf(status, legacy, message, galaxy = Build.MANUFACTURER.equals("samsung", ignoreCase = true)))
            }
        }
    }
    ...
    companion object {
        ...
        /**
         * 설치 실패를 사용자 안내로 바꾼다.
         * @param galaxy 갤럭시 기기인지. 중단된 설치에 '보안 위험 자동 차단' 안내와 버튼을 붙인다.
         */
        internal fun failureOf(status: Int, legacy: Int, message: String?, galaxy: Boolean): InstallEvent.Failed = InstallEvent.Failed(
            reason = describe(status, legacy, message, galaxy),
            detail =
            listOfNotNull(
                "상태 $status",
            ...
            isBlockedByGalaxySecurity(message) ||
                (galaxy && status == PackageInstaller.STATUS_FAILURE_ABORTED && legacy !in VERIFICATION_CODES),
        )
         ...
         */
        private fun describe(status: Int, legacy: Int, message: String?, galaxy: Boolean): String = when {
